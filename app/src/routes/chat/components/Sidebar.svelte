<script lang="ts">
    ...
    function print(peer: Peer) {
        console.log(peer == get(selectedPeer))
    }
</script>
...
<div class="w-full h-full flex flex-col p-4 gap-5 border-r border-gray-500">
		...
		<ul class="peer-list flex flex-col">
			{#each peers as peer}
				<div class="flex flex-row gap-2 mb-1 mr-2">
					<button class="w-full" on:click={() => {selectedPeer.set(peer); print(peer)}}>
						<PeerTile {peer} />
					</button>
	...
	<!-- Settings button -->
	<button on:click={logout} type="button" class="btn mt-auto">
