<script lang='ts'>
    ...
    selectedPeer.set(peers[0] ? peers[0] : 'No peer is selected');
    ...
    const addPeerToPeerList = () => {
        savedPeers.update(peers => [...peers, addPeerInput]);
        selectedPeer.set(addPeerInput);
        addPeerInput = '';
        toggleAddPeerActive();
        connect();
    };
...
<div class='w-full h-full flex flex-col p-4 gap-5 border-r border-gray-500'>
    ...
    <!-- Peer list -->
    <div class='peer-list-container mt-5 flex flex-col overflow-y-auto'>
    ...
    <!-- Settings button -->
    <button type="button" class="btn mt-auto">
